	...
	var Model = function (m) {
		...
		if (self.savedSearches.length === 0) {
			IP.data.ajax({
				...
				error: function () {
					IP.frameMessaging().dFrame.IP.message.error.raise("Unable to retrieve the saved searches. Please contact the system administrator.");
					self.savedSearches([]);
				}
			});
		}
		...
		if (self.workspaces.length === 0) {
			IP.data.ajax({
				...
				error: function () {
					IP.frameMessaging().dFrame.IP.message.error.raise("Unable to retrieve the workspace information. Please contact the system administrator.");
					self.workspaces([]);
				}
			});
		}
