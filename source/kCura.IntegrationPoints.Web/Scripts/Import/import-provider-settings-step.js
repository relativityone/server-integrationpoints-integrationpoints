...
(function (windowObj, root, ko) {
	...
	var currentLoadFileSettings = function () {
		var model = {
			...
			LoadFile: windowObj.RelativityImport.koModel.LoadFile(),
			LineNumber: windowObj.RelativityImport.koModel.startLine(),
			DestinationFolderArtifactId: windowObj.RelativityImport.koModel.DestinationFolderArtifactId()
		}
		return model;
	}
	...
	message.subscribe('loadFullState', function (fullModel) {
		...
		if (!!model) {
			...
			destinationFolderId = windowObj.RelativityImport.GetCachedUiModel.DestinationFolderArtifactId;
		};
		...
		windowObj.RelativityImport.getFolderAndSubFolders(destinationFolderId);
