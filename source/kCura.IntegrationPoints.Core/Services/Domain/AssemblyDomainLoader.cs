...
namespace kCura.IntegrationPoints.Core.Domain
{
	...
	public class AssemblyDomainLoader : MarshalByRefObject
	{
		...
		public static System.Reflection.Assembly ResolveAssembly(object sender, ResolveEventArgs args)
		{
			...
			lock (_assemblies)
			{
				...
				if (!_assemblies.ContainsKey(dllName))
				{
					...
					{
						returnedAssembly = ResolveMergedAssembly(dllName);
					}
		...
		public static System.Reflection.Assembly ResolveMergedAssembly(string dllName)
		{
			System.Reflection.Assembly returnedAssembly = null;
			if (MergedBinariesList.Any(s => { return s.Equals(dllName, StringComparison.InvariantCultureIgnoreCase); }))
			{
				string dllPath = Path.Combine(AppDomain.CurrentDomain.SetupInformation.PrivateBinPath, "kCura.IntegrationPoints.dll");
				if (File.Exists(dllPath))
				{
					returnedAssembly = System.Reflection.Assembly.LoadFile(dllPath);
				}
