...
namespace kCura.IntegrationPoints.Management.Tests.Tasks.Helpers
{
	...
	public class StuckJobsTests : TestBase
	{
		...
		public void ItShouldFindStuckJobs()
		{
			...
			var rdo2 = new RelativityObject
			{
				...
				FieldValues = new List<FieldValuePair>
				{
					new FieldValuePair
					{
						...
						Value = batchInstance2
					},
					new FieldValuePair
					{
						Field = new Field
						{
							Name = "SystemLastModifiedOn"
						},
						Value = now.AddHours(-2)
					}
				}
			};
			var rdo3 = new RelativityObject
			{
				ArtifactID = 593,
				FieldValues = new List<FieldValuePair>
				{
			...
			_runningJobRepository.GetRunningJobs(_WORKSPACE_ID_A).Returns(new List<RelativityObject>());
			_runningJobRepository.GetRunningJobs(_WORKSPACE_ID_B).Returns(new List<RelativityObject> { rdo2, rdo3 });
