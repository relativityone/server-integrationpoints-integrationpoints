...
namespace kCura.IntegrationPoints.Core.Tests.Services
{
    ...
    public class AgentLauncherTests : TestBase
    {
        ...
        public override void SetUp()
        {
            ...
            _sut = new AgentLauncher(_servicesMgrMock.Object, _logMock.Object, _toggleProviderMock.Object);
        }
        ...
        {
            // Arrange
            _toggleProviderMock.Setup(x => x.IsEnabled<EnableAgentLaunchOnJobStartToggle>()).Returns(true);

            // Act
            await _sut.LaunchAgentAsync();

            // Assert
            _agentStatusManagerServiceMock.Verify(x => x.StartAgentAsync(Guid.Parse(GlobalConst.RELATIVITY_INTEGRATION_POINTS_AGENT_GUID)), Times.Once);
        }
        ...
        {
            // Arrange
            _toggleProviderMock.Setup(x => x.IsEnabled<EnableAgentLaunchOnJobStartToggle>()).Returns(false);

            // Act
            await _sut.LaunchAgentAsync();

            // Assert
            _agentStatusManagerServiceMock.Verify(x => x.StartAgentAsync(Guid.Parse(GlobalConst.RELATIVITY_INTEGRATION_POINTS_AGENT_GUID)), Times.Never);
        }
    }
}
