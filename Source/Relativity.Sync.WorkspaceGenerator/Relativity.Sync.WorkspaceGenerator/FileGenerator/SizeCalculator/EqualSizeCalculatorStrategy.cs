...
namespace Relativity.Sync.WorkspaceGenerator.FileGenerator.SizeCalculator
{
	...
	{
		public IEnumerable<long> GetSizesInBytes(int count, long totalSizeInMB)
		{
			long totalSizeInBytes = totalSizeInMB * 1024 * 1024;
			long singleFileSizeInBytes = totalSizeInBytes / count;

			return Enumerable.Repeat(singleFileSizeInBytes, count);
		}
	}
}