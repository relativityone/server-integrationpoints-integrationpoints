	...
	$(function () {
		...
		hub.client.updateIntegrationPointData = function (integrationPoint, buttonStates, onClickEvents, sourceProviderIsRelativity) {
			...
			if (buttonStates.RunButtonEnabled) {
                ...
                $(consoleContainer.find(":contains('" + STOP + "')"))
                    .removeClass("consoleButtonDestructive")
                    .removeClass("consoleButtonDisabled")
                    .addClass("consoleButtonEnabled")
                    .attr("onClick", runOnClick)
                    .attr("title", RUN).html(RUN)
                    .removeAttr('disabled');

			} else if (buttonStates.StopButtonEnabled) {
				var stopClick = onClickEvents.StopOnClickEvent;
                $(consoleContainer.find(":contains('" + RUN + "')"))
                    .removeClass("consoleButtonEnabled")
                    .addClass("consoleButtonDestructive")
                    .attr("onClick", stopClick)
                    .attr("title", STOP)
                    .html(STOP);
			} else {
                ...
                $(consoleContainer.find(":contains('" + STOP + "')"))
                    .removeClass("consoleButtonDestructive")
                    .addClass("consoleButtonDisabled").removeAttr('onClick');
			}
			...
			if (sourceProviderIsRelativity) {
				if (buttonStates.RetryErrorsButtonEnabled) {
                    ...
                    $(consoleContainer.find(":contains('" + RETRY_ERRORS + "')"))
                        .removeClass("consoleButtonDisabled")
                        .addClass("consoleButtonEnabled")
                        .attr("onClick", retryErrorsClick)
                        .attr("title", RETRY_ERRORS)
                        .removeAttr('disabled');
				} else {
                    $(consoleContainer.find(":contains('" + RETRY_ERRORS + "')"))
                        .removeClass("consoleButtonEnabled")
                        .addClass("consoleButtonDisabled")
                        .removeAttr('onClick');
				}
				if (buttonStates.ViewErrorsLinkEnabled) {
					var viewErrorsClick = onClickEvents.ViewErrorsOnClickEvent;
                    $(consoleContainer.find(":contains('" + VIEW_ERRORS + "')"))
                        .removeClass("consoleLinkDisabled")
                        .addClass("consoleLinkEnabled")
                        .attr("onClick", viewErrorsClick)
                        .removeAttr('disabled');
				} else {
                    $(consoleContainer.find(":contains('" + VIEW_ERRORS + "')"))
                        .removeClass("consoleLinkEnabled")
                        .addClass("consoleLinkDisabled")
                        .removeAttr('onClick');
				}
			}
			...
			if (buttonStates.DownloadErrorFileLinkVisible) {
				...
				if (hasErrors) {
                    $(consoleContainer.find(":contains('" + DOWNLOAD_ERROR_FILE + "')"))
                        .removeClass("consoleLinkDisabled")
                        .addClass("consoleLinkEnabled")
                        .attr("onClick", downloadErrorsClick)
                        .removeAttr('disabled');
				} else {
                    $(consoleContainer.find(":contains('" + DOWNLOAD_ERROR_FILE + "')"))
                        .removeClass("consoleLinkEnabled")
