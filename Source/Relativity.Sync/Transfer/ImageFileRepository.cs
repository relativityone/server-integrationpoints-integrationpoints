...
namespace Relativity.Sync.Transfer
{
    internal class ImageFileRepository : IImageFileRepository
    {
        ...
        public ImageFileRepository(ISourceServiceFactoryForUser serviceFactoryForUser, IAPILog logger, SyncJobParameters parameters)
        {
            ...
            int workspaceId, int[] documentIds, QueryImagesOptions options)
        {
            ...
            _logger.LogInformation(
                "Found {numberOfImages} image files for {numberOfDocuments} documents",
                imageFiles.Length,
            ...
            int workspaceId, int[] documentIds, QueryImagesOptions options)
        {
            ...
            _logger.LogInformation(
                "Image retrieve statistics: ProducedImages: {producedImagesCount}, OriginalImages: {originalImagesCount}, DocumentsWithoutImages: {noImagesCount}",
        ...
        private async Task<(ImageFile[] Images, int[] DocumentWithoutImages)> RetrieveOriginalImagesForDocuments(int workspaceId, int[] documentIds)
        {
            ...
            if (dataSet == null || dataSet.Unwrap().Tables.Count == 0)
            {
                ...
                _logger.LogInformation(
                    "Image retrieve statistics: OriginalImages: {originalImagesCount}, DocumentsWithoutImages: {noImagesCount}",
