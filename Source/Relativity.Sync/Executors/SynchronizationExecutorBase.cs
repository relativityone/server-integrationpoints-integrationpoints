		...
		where TConfiguration : ISynchronizationConfiguration
	{
		...
		private IMetric GetBatchPerformanceMetric(int batchId, int savedSearchId, BatchProcessResult batchProcessResult, TimeSpan importApiTimer)
		{
			var metric = new BatchEndPerformanceMetric
			{
				...
				JobSizeGB = UnitsConverter.BytesToMegabytes(batchProcessResult.BytesTransferred),
				JobSizeGB_Metadata = UnitsConverter.BytesToMegabytes(batchProcessResult.MetadataBytesTransferred),
				JobSizeGB_Files = UnitsConverter.BytesToMegabytes(batchProcessResult.FilesBytesTransferred),
