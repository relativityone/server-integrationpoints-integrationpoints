...
#pragma warning restore S1135 // Track uses of "TODO" tags
				...
				catch (ServiceException ex)
				{
					...
					throw new KeplerServiceException($"Service call failed while querying {nameof(DestinationWorkspaceTag)} in workspace {sourceWorkspaceArtifactId}", ex);
				}
				catch (Exception ex)
				{
					_logger.LogError(ex, $"Failed to query {nameof(DestinationWorkspaceTag)} object: {{request}}", request);
					throw new KeplerServiceException($"Failed to query {nameof(DestinationWorkspaceTag)} in workspace {sourceWorkspaceArtifactId}", ex);
				}
...
#pragma warning restore S1135 // Track uses of "TODO" tags
			...
			using (IObjectManager objectManager = await _sourceServiceFactoryForUser.CreateProxyAsync<IObjectManager>().ConfigureAwait(false))
			{
				...
				catch (ServiceException ex)
				{
					...
					throw new KeplerServiceException($"Service call failed while creating {nameof(DestinationWorkspaceTag)}: {request}", ex);
				}
				catch (Exception ex)
				{
					...
					throw new KeplerServiceException(
		...
		public async Task UpdateAsync(int sourceWorkspaceArtifactId, DestinationWorkspaceTag destinationWorkspaceTag)
		{
			...
			using (IObjectManager objectManager = await _sourceServiceFactoryForUser.CreateProxyAsync<IObjectManager>().ConfigureAwait(false))
			{
				...
				catch (ServiceException ex)
				{
					...
					throw new KeplerServiceException($"Failed to update {nameof(DestinationWorkspaceTag)} with id {destinationWorkspaceTag.ArtifactId} in workspace {sourceWorkspaceArtifactId}", ex);
				}
				catch (Exception ex)
				{
