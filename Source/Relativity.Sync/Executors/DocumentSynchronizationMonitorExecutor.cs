...
namespace Relativity.Sync.Executors
{
    internal class DocumentSynchronizationMonitorExecutor : IExecutor<IDocumentSynchronizationMonitorConfiguration>
    {
            ...
            try
            {
                    ...
                    .ConfigureAwait(false))
                {
                    ...
                    do
                    {
                        ...
                        await HandleDataSourceStatusAsync(batches).ConfigureAwait(false);

                        result = await _importService.GetJobImportStatusAsync().ConfigureAwait(false);
                    ...
                    return GetFinalJobStatus(batches, result);
                }
            }
            catch (Exception ex)
            {
        ...
        private async Task HandleDataSourceStatusAsync(List<IBatch> batches)
        {
            foreach (IBatch batch in batches.Where(x => !x.IsFinished))
            {
                DataSourceDetails dataSourceDetails = await _importService.GetDataSourceStatusAsync(batch.BatchGuid).ConfigureAwait(false);

                ImportProgress dataSourceProgress = await _importService.GetDataSourceProgressAsync(batch.BatchGuid).ConfigureAwait(false);
                ...
                if (dataSourceDetails.IsFinished())
                {
                    ...
                    await EndBatchAsync(batch, dataSourceDetails, dataSourceProgress).ConfigureAwait(false);
                }
            }
        }

        private ExecutionResult GetFinalJobStatus(List<IBatch> batches, ImportDetails importDetails)
        {
            switch (importDetails.State)
            {
                case ImportState.Failed:
                    _logger.LogError("Import Errors: {@errors}", importDetails.Errors);
                ...
                default:
                    ...
                    _logger.LogError(errorMsg, importDetails.State);
                    return ExecutionResult.Failure(string.Format(errorMsg, importDetails.State));
            }
        }

        private async Task EndBatchAsync(IBatch batch, DataSourceDetails dataSourceDetails, ImportProgress dataSourceProgress)
        {
            switch (dataSourceDetails.State)
            {
                ...
                case DataSourceState.CompletedWithItemErrors:
                    await HandleDataSourceErrorsAsync(batch.BatchGuid).ConfigureAwait(false);
            ...
            await batch.SetTransferredDocumentsCountAsync(dataSourceProgress.ImportedRecords).ConfigureAwait(false);
            await batch.SetFailedDocumentsCountAsync(dataSourceProgress.ErroredRecords).ConfigureAwait(false);
        }

        private async Task HandleDataSourceErrorsAsync(Guid dataSourceId)
        {
            ...
            do
            {
                errors = await _importService.GetDataSourceErrorsAsync(dataSourceId, start, length).ConfigureAwait(false);
        ...
        private async Task CancelImportJob(Guid exportRunId)
        {
            ImportDetails status = await _importService.GetJobImportStatusAsync().ConfigureAwait(false);

            if (status.State != ImportState.Canceled)
            {
                _logger.LogInformation("Raise cancellation request for Import Job {jobId}.", exportRunId);
