	...
	internal sealed class SynchronizationExecutorTests : SystemTest
	{
		...
		public async Task ItShouldPassGoldFlow()
		{
			...
			IImportJobFactory importJobFactory = new Executors.ImportJobFactory(
				...
				logger);
			Storage.IConfiguration config = await Storage.Configuration.GetAsync(_serviceFactoryStub, new SyncJobParameters(jobHistoryArtifactId, sourceWorkspaceArtifactId, Guid.NewGuid(), 
					configuration.ImportSettings), logger, new SemaphoreSlimWrapper(new SemaphoreSlim(1))).ConfigureAwait(false);
