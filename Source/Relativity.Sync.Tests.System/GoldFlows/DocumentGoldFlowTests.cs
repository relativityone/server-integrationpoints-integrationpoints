...
namespace Relativity.Sync.Tests.System.GoldFlows
{
    ...
    internal sealed class DocumentGoldFlowTests : SystemTest
    {
        ...
        [IdentifiedTest("0BB3F87F-F12F-45FE-8340-A43262D04E19")]
        public async Task SyncJob_ShouldSyncDocuments_WithoutTagging()
        {
            // Arrange
            string tagValue = $"This Instance - {_goldFlowTestSuite.SourceWorkspace.Name} - {_goldFlowTestSuite.SourceWorkspace.ArtifactID}";
        ...
        private async Task ConfigureTestRunAsync(WorkspaceRef sourceWorkspace, WorkspaceRef destinationWorkspace, ConfigurationStub configuration)
        {
            ...
            List<FieldMap> extractedTextMapping = await GetExtractedTextMappingAsync(sourceWorkspace.ArtifactID, destinationWorkspace.ArtifactID).ConfigureAwait(false);

            configuration.SetFieldMappings(extractedTextMapping);
        }
    }
}
