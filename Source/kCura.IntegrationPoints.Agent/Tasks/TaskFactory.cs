...
namespace kCura.IntegrationPoints.Agent.Tasks
{
	public class TaskFactory : ITaskFactory
	{
		...
		private IntegrationPoint GetIntegrationPoint(Job job)
		{
			...
			var integrationPoint = _caseServiceContext.RsapiService.RelativityObjectManager.Read<IntegrationPoint>(job.RelatedObjectArtifactID);
		...
		private void UpdateJobHistoryOnFailure(Job job, IntegrationPoint integrationPointDto, Exception e)
		{
			...
			var healthcheck = Client.APMClient.HealthCheckOperation(Constants.IntegrationPoints.Telemetry.APM_HEALTHCHECK, () => HealthCheck.CreateJobFailedMetric(jobHistory, job.WorkspaceID));
			healthcheck.Write();
		...
		internal void RemoveJobHistoryFromIntegrationPoint(IntegrationPoint integrationPointDto, JobHistory jobHistory)
		{
			...
			_caseServiceContext.RsapiService.RelativityObjectManager.Update(integrationPointDto);
