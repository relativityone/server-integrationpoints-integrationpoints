    ...
    internal class ImportApiServiceTests
    {
        ...
        public void SetUp()
        {
            ...
            _importJobControllerMock
                .Setup(x => x.CreateAsync(_importJobContext.WorkspaceId, _importJobContext.ImportJobId, RIP_APPLICATION_NAME, _importJobContext.RipJobId.ToString()))
                .ReturnsAsync(_successResponse);
            _importJobControllerMock
                .Setup(x => x.BeginAsync(_importJobContext.WorkspaceId, _importJobContext.ImportJobId))
                .ReturnsAsync(_successResponse);

            _documentConfigurationControllerMock = new Mock<IDocumentConfigurationController>();
            _documentConfigurationControllerMock
                .Setup(x => x.CreateAsync(_importJobContext.WorkspaceId, _importJobContext.ImportJobId, _documentConfiguration.DocumentSettings))
                .ReturnsAsync(_successResponse);

            _rdoConfigurationControllerMock = new Mock<IRDOConfigurationController>();
            _rdoConfigurationControllerMock
                .Setup(x => x.CreateAsync(_importJobContext.WorkspaceId, _importJobContext.ImportJobId, _rdoConfiguration.RdoSettings))
                .ReturnsAsync(_successResponse);

            _advancedConfigurationControllerMock = new Mock<IAdvancedConfigurationController>();
            _advancedConfigurationControllerMock
                .Setup(x => x.CreateAsync(_importJobContext.WorkspaceId, _importJobContext.ImportJobId, It.IsAny<AdvancedImportSettings>()))
                .ReturnsAsync(_successResponse);
        ...
        {
            _advancedConfigurationControllerMock
                .Setup(x => x.CreateAsync(_importJobContext.WorkspaceId, _importJobContext.ImportJobId, _documentConfiguration.AdvancedSettings))
                .ReturnsAsync(_failedResponse);
        ...
        {
            _advancedConfigurationControllerMock
                .Setup(x => x.CreateAsync(_importJobContext.WorkspaceId, _importJobContext.ImportJobId, _rdoConfiguration.AdvancedSettings))
                .ReturnsAsync(_failedResponse);
