...
namespace Relativity.Sync.Tests.Unit.Telemetry.Metrics
{
    internal class NonDocumentJobEndMetricsTests : MetricTestsBase<NonDocumentJobEndMetric>
    {
        ...
        protected override void VerifySumSink(Mock<IMetricsManager> metricsManagerMock)
        {
            ...
            metricsManagerMock.Verify(x => x.LogPointInTimeStringAsync(
                TelemetryConstants.MetricIdentifiers.JOB_END_STATUS_NON_DOCUMENT_OBJECTS,
            ...
            metricsManagerMock.Verify(x => x.LogPointInTimeLongAsync(
                TelemetryConstants.MetricIdentifiers.DATA_FIELDS_LINKS_MAPPED,
                _EXPECTED_WORKSPACE_GUID, _sut.CorrelationId, _sut.TotalLinksMappedFields.Value));
            metricsManagerMock.Verify(x => x.LogPointInTimeLongAsync(
                TelemetryConstants.MetricIdentifiers.DATA_FIELDS_TOTAL_REQUESTED,
        ...
        protected override void VerifyApmSink(Mock<IAPMClient> apmMock)
        {
            apmMock.Verify(x => x.Count(_APPLICATION_NAME, It.Is<Dictionary<string, object>>(d =>
                ...
                d["TotalLinksMappedFields"].Equals(_sut.TotalLinksMappedFields) &&
