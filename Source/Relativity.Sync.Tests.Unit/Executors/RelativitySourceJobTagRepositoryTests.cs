...
namespace Relativity.Sync.Tests.Unit.Executors
{
	...
	public sealed class RelativitySourceJobTagRepositoryTests
	{
		...
		public void SetUp()
		{
			...
			_sut = new RelativitySourceJobTagRepository(_serviceFactory.Object, _tagNameFormatter.Object, new EmptyLogger());
		}
		...
		public async Task ItShouldCreateDestinationWorkspaceTag()
		{
			...
			_tagNameFormatter.Setup(x => x.FormatSourceJobTagName(It.IsAny<string>(), It.IsAny<int>())).Returns(sourceJobTagName);
		...
		[Test]
		public void ItShouldThrowRepositoryExceptionWhenJobHistoryNameQueryFails()
		{
			_objectManager.Setup(x => x.QueryAsync(It.IsAny<int>(), It.IsAny<QueryRequest>(), It.IsAny<int>(), It.IsAny<int>(), CancellationToken.None, It.IsAny<IProgress<ProgressReport>>()))
