...
namespace Relativity.Sync.Tests.Unit.Executors
{
	...
	internal class ImageSynchronizationExecutorTests
	{
		...
		public async Task Execute_ShouldSendBatchMetrics()
		{
			...
			_batchRepositoryMock.Setup(x => x.GetAsync(It.IsAny<int>(), It.IsAny<int>(), It.IsAny<Guid>())).ReturnsAsync(batchStub);
		...
		public async Task Execute_ShouldRunImportApiJobForEachBatch(int numberOfBatches)
		{
			...
			// Assert
			_batchRepositoryMock.Verify(x => x.GetAsync(It.IsAny<int>(), It.IsAny<int>(), It.IsAny<Guid>()), Times.Exactly(numberOfBatches));
		...
		public static IEnumerable<Action<ImageSynchronizationExecutorTests, ExecutionResult>> BrakingActionsSetups => new Action<ImageSynchronizationExecutorTests, ExecutionResult>[]
		{
		...
		public async Task Execute_ShouldBreak_WhenPushingOrTaggingBrakes(
			...
			[ValueSource(nameof(BrakingActionsSetups))] Action<ImageSynchronizationExecutorTests, ExecutionResult> brakingActionSetup)
		{
			// arrange
			const int numberOfBatches = 1;
			SetupBatchRepository(numberOfBatches);
		...
		private void SetupBatch(IBatch batch)
		{
			...
			_batchRepositoryMock.Setup(x => x.GetAsync(It.IsAny<int>(), It.IsAny<int>(), It.IsAny<Guid>()))
				.ReturnsAsync(batch);
		}
