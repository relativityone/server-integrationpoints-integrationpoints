...
namespace Relativity.Sync.Tests.Unit
{
	...
	public sealed class ConfigurationCacheTests
	{
		...
		public async Task ItShouldReadFields()
		{
			...
			// ACT
			IConfigurationCache cache = await ConfigurationCache.GetAsync(_serviceFactory, _WORKSPACE_ID, _ARTIFACT_ID, new EmptyLogger(), Mock.Of<ISemaphoreSlim>()).ConfigureAwait(false);
		...
		public void ItShouldFailWhenConfigurationNotFound()
		{
			...
			// ACT
			Func<Task> action = async () => await ConfigurationCache.GetAsync(_serviceFactory, _WORKSPACE_ID, _ARTIFACT_ID, new EmptyLogger(), Mock.Of<ISemaphoreSlim>()).ConfigureAwait(false);
		...
		public async Task ItShouldDisposeSemaphore()
		{
			...
			IConfigurationCache cache = await ConfigurationCache.GetAsync(_serviceFactory, _WORKSPACE_ID, _ARTIFACT_ID, new EmptyLogger(), semaphore.Object).ConfigureAwait(false);
