namespace kCura.IntegrationPoints.Core.Contracts.BatchReporter
{
	public class NullBatchReporter : IBatchReporter
	{
		...
		{
			add { _onBatchComplete += value; }
			remove { _onBatchComplete -= value; }
		}
		...
		{
			add { _onBatchCreate += value; }
			remove { _onBatchCreate -= value; }
		}

		public event StatusUpdate OnStatusUpdate
		{
			add { _onStatusUpdate += value; }
			remove { _onStatusUpdate -= value; }
		}
		...
		{
			add { _onDocumentError += value; }
			remove { _onDocumentError -= value; }
		}
	}
}
